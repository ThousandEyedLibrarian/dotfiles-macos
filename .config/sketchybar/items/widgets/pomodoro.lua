...
local config = {
	...
	long_break_every = 3, -- long break after every N work phases
...
local function phase_base_color()
	...
	if state.phase == "short_break" then
		return colors.green
	end
	return colors.blue
end
...
local function notify(msg)
	if not config.notify_on_finish then
		return
	end
	...
	if sound ~= "" then
		...
		sbar.exec(string.format([[osascript -e 'display notification "%s" with title "Pomodoro"']], msg))
	end
end
...
settings_btn:subscribe("mouse.clicked", function()
	sbar.exec(
		[[open -na Ghostty --args -e /bin/zsh -c "${EDITOR:-nvim} ~/.config/sketchybar/items/widgets/pomodoro.lua"]]
	)
	close_popup()
end)
